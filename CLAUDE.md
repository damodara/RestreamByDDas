...
- **Stream statistics + server load (implemented)**: see Architecture below. Restarting a broadcast is still not built (the `push.sh`/`stop.sh` PID-file mechanism is a natural hook point for it, but no UI/command uses it yet).
