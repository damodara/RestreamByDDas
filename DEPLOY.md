...
Этот стек сам по себе не терминирует TLS — nginx здесь настроен только для RTMP-приёма и plain HTTP-проксирования на порт 80, сертификатов и HTTPS в нём нет (см. CLAUDE.md — сознательно отложено). Значит:
