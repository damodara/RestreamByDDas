...
- Рестрим (фан-аут) на произвольное число RTMP-дестинаций, с пресетами популярных площадок и тестовым пушем (проверка адреса/ключа без реального эфира, поодиночке или сразу все)
- Регистрация с подтверждением администратором по email-ссылке (без самостоятельной регистрации)
- Личный кабинет: смена логина и email (с подтверждением по ссылке на новый адрес), пароль, срок хранения логов, уведомления на email и/или в Telegram об ошибке рестрима и о необъявленном обрыве приёма (без явного «Завершить эфир»)
- Встроенный чат трансляции (YouTube Live Chat)
- Статистика потока (битрейт, аптайм, кодеки) и нагрузка сервера
- Перезапуск трансляции (сброс паблишера) и явное завершение эфира — подхватывает изменения списка дестинаций
